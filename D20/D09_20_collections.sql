...
begin
    --APEX_COLLECTION.DELETE_COLLECTION ('D20_FATTURA_TESTATA');
if not apex_collection.collection_exists('D20_FATTURA_TESTATA') then
    apex_collection.create_collection('D20_FATTURA_TESTATA');
  for r in (select id,
                   cliente_id,
                   anno,
                   numero,
                   descrizione,
                   data 
              from D20_FATTURA_testata
        )
  loop
    apex_collection.add_member(p_collection_name=>'D20_FATTURA_TESTATA'
                              ,p_n001 => r.id
                              ,p_n002 => r.cliente_id
                              ,p_c001 => r.anno
                              ,p_c002 => r.numero
...
select seq_id seq_id
      ...
      ,c003 descrizione
      ,d001 data 
  from apex_collections where collection_name='D20_FATTURA_TESTATA';


APEX_COLLECTION.ADD_MEMBER (
                               p_collection_name=>'D20_FATTURA_TESTATA'
                              ,p_n001 => :P26_id
